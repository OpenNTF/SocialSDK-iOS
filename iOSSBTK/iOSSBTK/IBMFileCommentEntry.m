...
#import "IBMFileCommentEntry.h"
#import "IBMConstants.h"
#import "FBLog.h"

@interface IBMFileCommentEntry ()

@property (strong, nonatomic) NSMutableDictionary *xpathMap;

@end

@implementation IBMFileCommentEntry
...
- (IBMFilePersonEntry *)personEntry {
    if (_personEntry == nil) {
        _personEntry = [[IBMFilePersonEntry alloc] initWithXMLDocument:self.xmlDocument userType:FP_AUTHOR];
    }
    
    return _personEntry;
}

- (IBMFilePersonEntry *)modifier {
    if (_modifier == nil) {
        _modifier = [[IBMFilePersonEntry alloc] initWithXMLDocument:self.xmlDocument userType:FP_MODIFIER];
    }
...
- (id) getFieldWithXPath:(NSString *) fieldName {
    ...
    NSError *error;
    NSDictionary *dict = [IBMFileCommentEntry namespacesForCommentEntry];
