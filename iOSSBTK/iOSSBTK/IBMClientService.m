...
@property (strong, nonatomic) IBMHttpClient *httpClient;

@end

@implementation IBMClientService
...
- (void) initializeEnv {
    ...
    if ([[self.endPoint getAuthType] isEqualToString:BASICAUTH]){
        ...
        self.httpClient = [[IBMHttpClient alloc] initWithBaseURL:[NSURL URLWithString:[IBMUtils getUrlForEndPoint:self.endPoint.endPointName]]];
...
- (void) initGetRequestWithPath:(NSString *) path
    ...
    [self.httpClient getPath:path parameters:param success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (formattedResult == nil && type != RESPONSE_NONE) {
            failure(operation.response, [NSError errorWithDomain:@"IBMClientService"
                                        code:100
...
- (void) initMultiPartFileUploadRequestWithPath:(NSString *) path
    ...
    if ([content isKindOfClass:[NSString class]]) {
        ...
        failure(nil, [NSError errorWithDomain:@"IBMClientService"
                                    code:100
...
- (id) processXMLResponseWithData:(id) data {
    NSError *error;
    IBMXMLDocument *doc = [[IBMXMLDocument alloc] initWithData:(NSData *) data options:0 error:&error];
