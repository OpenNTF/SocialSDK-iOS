...
@interface SBTFilePersonEntry ()

@property (strong, nonatomic) NSMutableDictionary *xpathMap;

@end

@implementation SBTFilePersonEntry
...
- (id) initWithXMLDocument:(SBTXMLDocument *)document userType:(FP_USER_TYPE) type_ {
    if (self = [super init]) {
        [self initEnv];
...
- (id) getFieldWithXPath:(NSString *) fieldName {
    ...
    NSError *error;
    NSDictionary *dict = [SBTFilePersonEntry namespacesForPersonEntry];
