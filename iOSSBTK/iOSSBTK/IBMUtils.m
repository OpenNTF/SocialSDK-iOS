...
+ (NSString *) getUrlForEndPoint:(NSString *) endPointName {
    ...
    if ([endPointName hasPrefix:@"connections"])
        url = [IBMCredentialStore loadWithKey:IBM_CREDENTIAL_CONNECTIONS_URL];
    else if ([endPointName hasPrefix:@"smartcloud"])
        url = [IBMCredentialStore loadWithKey:IBM_CREDENTIAL_SMARTCLOUD_URL];
