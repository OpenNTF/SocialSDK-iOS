...
#import "IBMCredentialStoreTest.h"
#import "IBMCredentialStore.h"

@implementation IBMCredentialStoreTest
...
- (void) testStoreGetAndRemove {
    ...
    // First create
    BOOL result = [IBMCredentialStore storeWithKey:testCaseKey value:testCaseValue];
    if (result == NO) {
        STFail(@"Unable to store key value pair");
    }
    
    // Second retrieve
    NSString *value = [IBMCredentialStore loadWithKey:testCaseKey];
    ...
    // Third delete
    result = [IBMCredentialStore removeWithKey:testCaseKey];
    if (result == NO) {
